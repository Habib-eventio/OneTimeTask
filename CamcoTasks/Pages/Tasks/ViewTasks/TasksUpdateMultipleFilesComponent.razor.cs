...
{
    public partial class TasksUpdateMultipleFilesComponent
    {
        [Inject]
        ...
        [Parameter]
        public TasksTaskUpdatesViewModel TaskViewModel { get; set; }
        [Parameter]
        ...
        protected async Task LoadData()
        {
            ...
            {
                TaskFiles = (await taskService.GetOneTimeTaksUpdateFilesAsync1(TaskViewModel.UpdateId)).ToList();
            }
        }
        ...
        protected async Task ViewFiles(TasksTaskUpdatesViewModel Model)
        {
            if (Model != null)
            {
                isActiveImageViewComponent = true;
        ...
        protected async Task SuccessMessageFromDeleteFileComponent(TasksImagesViewModel model)
        {
            if (model != null)
            {
                ...
                if (TaskFiles == null || !TaskFiles.Any())
                {
                    var updateTask = TaskViewModel;
                    updateTask.IsPicture = false;
                    var result = await taskService.UpdateTaskUpdate(updateTask);
