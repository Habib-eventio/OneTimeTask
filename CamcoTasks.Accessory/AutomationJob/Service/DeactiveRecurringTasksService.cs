...
{
    public class DeactiveRecurringTasksService : IDeactiveRecurringTasksService
    {
        ...
        public DeactiveRecurringTasksService(ILogger<DeactiveRecurringTasksService> logger,
            IAutomatedAutomationsService automatedAutomationsService, 
            IRecurringTaskReportService recurringTaskReportService)
        {
        ...
        public async Task DoWork(CancellationToken stoppingToken)
        {
            ...
            {
                await DeactiveTasks(stoppingToken);
            }
        }

        private async Task DeactiveTasks(CancellationToken stoppingToken)
        {
            try
            {
                var automation = await _automatedAutomationsService.GetAutomationAsync("Deactive Tasks");
                        ...
                        if (automation.LastRun.Date != today.Date || automation.NeedsRestart)
                        {
                                ...
                                || automation.NeedsRestart)
                            {
                                if (automation.EarliestTime < today.TimeOfDay
                                    && today.TimeOfDay < automation.LatestTime
                                    || automation.NeedsRestart)
                                {
                                    await _recurringTaskReportService
                                    .SendWarningMailBeforeDeactiveTaskAsync(AppInformation.AppUrl);
                                    await _recurringTaskReportService.DeactivateTasksAsync();
