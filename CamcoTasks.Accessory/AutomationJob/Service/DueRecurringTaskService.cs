...
{
    public class DueRecurringTaskService : IDueRecurringTaskService
    {
        ...
        public DueRecurringTaskService(ILogger<DueRecurringTaskService> logger,
        ...
        private async Task DueRecurringTask(CancellationToken stoppingToken)
        {
            try
            {
                var automation = await _automatedAutomationsService.GetAutomationAsync("Due Recurring Task");
                                ...
                                || automation.NeedsRestart)
                            {
                                    ...
                                    && today.TimeOfDay < automation.LatestTime || automation.NeedsRestart)
                                {
                                    await _recurringTaskReportService
                                        .CheckPastDueTasksAsync(AppInformation.AppUrl);
