...
{
    public class RecTasksDueDateReportService : IRecTasksDueDateReportService
    {
        ...
        public RecTasksDueDateReportService(ILogger<DueRecurringTaskService> logger,
        ...
        public async Task DoWork(CancellationToken stoppingToken)
        {
            ...
            {
                await RecurringTaskFrequencyDueDateReport(stoppingToken);
            }
        }

        private async Task RecurringTaskFrequencyDueDateReport(CancellationToken stoppingToken)
        {
            try
            {
                var automation = await _automatedAutomationsService
                    .GetAutomationAsync("Recurring Task Due Date Report");
                        ...
                        if (automation.LastRun.Date != today.Date || automation.NeedsRestart)
                        {
                                ...
                                || automation.NeedsRestart)
                            {
                                if ((automation.EarliestTime < today.TimeOfDay
                                    ...
                                    && (today.Day == 15 || today.Day == 30))
                                    || automation.NeedsRestart)
                                {
                                    await _recurringTaskReportService
                                        .CheckPastDueTasksAsync(DateTime.Now);
