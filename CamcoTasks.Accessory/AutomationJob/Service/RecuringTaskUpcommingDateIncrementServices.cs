...
{
    public class RecuringTaskUpcommingDateIncrementServices : IRecuringTaskUpcommingDateIncrementServices
    {
        ...
        public RecuringTaskUpcommingDateIncrementServices(ILogger<DueRecurringTaskService> logger,
            IAutomatedAutomationsService automatedAutomationsService, IRecurringTaskReportService recurringTaskReportService)
        {
        ...
        public async Task DoWork(CancellationToken stoppingToken)
        {
            ...
            {
                await IncrementRecurringTaskUpcomincDate(stoppingToken);
            }
        }

        private async Task IncrementRecurringTaskUpcomincDate(CancellationToken stoppingToken)
        {
            try
            {
                var automation = await _automatedAutomationsService
                    .GetAutomationAsync("Recurring Task Upcoming Date Increment");
                ...
                if (automation != null)
                {
                    if (automation.IsEnabled)
                    {
                        ...
                        if (automation.LastRun.Date != today.Date || automation.NeedsRestart)
                        {
                            ...
                            {
                                if (automation.EarliestTime < today.TimeOfDay
                                    && today.TimeOfDay < automation.LatestTime || automation.NeedsRestart)
                                {
                                    await _recurringTaskReportService
                                        .IncressUpcommicDate("MONTHLY", 14);
                                    await _recurringTaskReportService
                                        .IncressUpcommicDate("QUARTERLY", 14);
                                    await _recurringTaskReportService
                                        .IncressUpcommicDate("TWICE/YEAR", 14);
                                    await _recurringTaskReportService
                                        .IncressUpcommicDate("YEARLY", 14);
